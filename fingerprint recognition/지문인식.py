...
from imgaug import augmenters as iaa  # 이미지를 변형시켜 프로그램이 다양한 상황에서 대처 할 수 있게 해주는 패키지
...
seq = iaa.Sequential([
    ...
    iaa.Affine(
        # scale images to 9- 110% of their size, individually per axis
        scale={"x": (0.9, 1.1), "y": (0.9, 1.1)},
        # translate by -10 to +10 percent (per axis)
        translate_percent={"x": (-0.1, 0.1), "y": (-0.1, 0.1)},
        # rotate by -30 ro +30 degrees
        rotate=(-30, 30),
...
class DataGenerator(keras.utils.data_utils.Sequence):
    ...
    def on_epoch_end(self):
        if self.shuffle == True: self.x, self.label = shuffle(self.x, self.label)
...
# feature models that sharing weights
...
random_img = seq.augment_images(random_img).reshape((1, 90, 90, 1)).astype(np.float32) / 255
